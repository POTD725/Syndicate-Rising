...
var selected_room := "backroom"
var message := "Tap a building to inspect it."
var elapsed := 0.0
var tick_clock := 0.0

func _ready() -> void:
	var ids := ["backroom", "chop_shop", "black_market", "bunks", "clinic", "boss_office", "signal_den", "tunnel"]
	for index in range(ids.size()):
		var column := index % 2
		var row := index / 2
		building_rects[ids[index]] = Rect2(18.0 + column * 351.0, 146.0 + row * 194.0, 333.0, 180.0)
	button_rects = {
...
func _input(event: InputEvent) -> void:
	...
	if not pressed:
		return
	for key in building_rects:
		if (building_rects[key] as Rect2).has_point(pos):
			selected_room = String(key)
			var room := SyndicateState.get_room(selected_room)
			message = "%s selected." % room.get("name", "Building")
			SyndicateAudio.play_sfx("click")
			return
	for key in button_rects:
		if (button_rects[key] as Rect2).has_point(pos):
			_action(String(key))
			return
...
func _action(action: String) -> void:
	...
	match action:
		...
		"load":
			...
			SyndicateAudio.play_sfx("accept" if bool(result.get("ok", false)) else "warning")
		"sound":
			var muted := SyndicateAudio.toggle_muted()
...
func _draw_background() -> void:
	...
	for index in range(120):
		var x := fmod(float(index * 97 + 23), VIEW.x)
		var y := fmod(float(index * 61 + 41), VIEW.y)
		draw_circle(Vector2(x, y), 0.8 + float(index % 3) * 0.45, Color("c9a9ff", 0.16))
	for row in range(5):
		var y := 130.0 + row * 194.0
		draw_line(Vector2(0.0, y), Vector2(VIEW.x, y), Color("bb69ef", 0.10), 1.0)
...
func _draw_header() -> void:
	...
	var stats := "CR %d   CARGO %d   INTEL %d" % [SyndicateState.credits, SyndicateState.contraband, SyndicateState.intel]
	...
	var tech_text := "READY" if SyndicateState.black_tech_end <= 0 else "%ds" % SyndicateState.seconds_left(SyndicateState.black_tech_end)
	...
	_draw_button(button_rects["sound"], "AUDIO OFF" if SyndicateAudio.muted else "AUDIO ON", false)

func _draw_buildings() -> void:
	for room in SyndicateState.rooms:
		var id := String(room.get("id", ""))
		var rect := building_rects[id] as Rect2
		var selected := id == selected_room
		var repaired := bool(room.get("repaired", false))
		var border := Color("fff3fb") if selected else (Color("c66dff") if repaired else Color("7a3b58"))
		draw_style_box(_panel(Color("190d21"), border, 3 if selected else 1, 12), rect)
		var art_rect := Rect2(rect.position + Vector2(5.0, 5.0), Vector2(rect.size.x - 10.0, 128.0))
		...
		var state_text := "ONLINE L%d" % int(room.get("level", 1))
		if not repaired:
		...
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(9.0, 169.0), "%s  •  %s" % [room.get("function", ""), state_text], HORIZONTAL_ALIGNMENT_LEFT, 310.0, 9, Color("71edcf") if repaired else Color("ff9a7d"))

func _draw_crew_activity() -> void:
	var path := [Vector2(90, 310), Vector2(620, 310), Vector2(620, 690), Vector2(90, 690), Vector2(90, 310)]
	for index in range(SyndicateState.crew.size()):
		var member := SyndicateState.crew[index]
		var phase := fmod(elapsed * (0.11 + index * 0.015) + index * 0.23, 1.0)
		var segment_float := phase * 4.0
		var segment := mini(3, int(segment_float))
		var local := segment_float - segment
		var pos := path[segment].lerp(path[segment + 1], local)
		var texture := PORTRAITS[String(member.get("id", "crew_1"))] as Texture2D
...
func _draw_inspector() -> void:
	var panel_rect := Rect2(18.0, 934.0, 684.0, 232.0)
	draw_style_box(_panel(Color("140b1b", 0.98), Color("a75bd0"), 2, 14), panel_rect)
	var room := SyndicateState.get_room(selected_room)
	var preview := Rect2(30.0, 950.0, 246.0, 146.0)
	...
	draw_string(ThemeDB.fallback_font, Vector2(294.0, 1004.0), "%s • LEVEL %d" % [room.get("function", ""), room.get("level", 1)], HORIZONTAL_ALIGNMENT_LEFT, 380.0, 12, Color("ff8cbd"))
	var status := "ONLINE" if bool(room.get("repaired", false)) else "DAMAGED"
	...
	var action_label := "UPGRADE" if bool(room.get("repaired", false)) else "REBUILD"
	_draw_button(button_rects["room_action"], action_label, true)
...
func _draw_navigation() -> void:
	...
	_draw_button(button_rects["scores"], "SCORES", true)
	_draw_button(button_rects["tech"], "BLACK TECH", false)
	_draw_button(button_rects["fence"], "FENCE", false)
	_draw_button(button_rects["save"], "SAVE", false)
	_draw_button(button_rects["load"], "LOAD", false)
