...
const SAVE_PATH: String = "user://syndicate_rising_save.json"
const MAX_ROOM_LEVEL: int = 10
const JOB_LIMIT: int = 3
const FINAL_STORY_CHAPTER: int = 5

var credits: int = 650
var contraband: int = 20
var intel: int = 12
var heat: int = 8
var notoriety: int = 1
...
var pending_cutscene: String = "prologue"
...
var last_event: String = "SYNDICATE RISING // Rebuild the district and take the Moon back from underneath."
...
func upgrade_room(room_id: String) -> Dictionary:
	...
	if level >= MAX_ROOM_LEVEL:
		return {"ok": false, "message": "%s is already max level." % String(room.get("name", "Building"))}
	var cost: int = 100 + level * 85
	if credits < cost:
		return {"ok": false, "message": "The level %d upgrade needs %d credits." % [level + 1, cost]}
	credits -= cost
	room["level"] = level + 1
	last_event = "%s upgraded to level %d." % [String(room.get("name", "Building")), level + 1]
	state_changed.emit()
	return {"ok": true, "message": last_event}
...
func begin_black_tech() -> Dictionary:
	...
	credits -= credit_cost
	intel -= intel_cost
	black_tech_end = _now() + maxi(8, 18 - get_room_level("signal_den"))
...
func fence_contraband() -> Dictionary:
	...
	var moved: int = mini(contraband, 5 + black_tech_level + get_room_level("tunnel"))
	var payout: int = moved * (22 + black_tech_level * 3 + get_room_level("tunnel") * 2)
	contraband -= moved
	credits += payout
...
func begin_job(job_id: String, crew_ids: Array[String]) -> Dictionary:
	...
	if is_room_repaired("boss_office"):
		crew_limit = mini(4, 2 + get_room_level("boss_office"))
	if valid_ids.size() > crew_limit:
		valid_ids.resize(crew_limit)
	active_job = jobs[job_index].duplicate(true) as Dictionary
	active_crew_ids = valid_ids
	jobs.remove_at(job_index)
	for crew_id: String in active_crew_ids:
		var assigned: Dictionary = get_crew_member(crew_id)
		assigned["busy_until"] = _now() + 45
...
func finish_job(victory: bool, surviving_hp: Dictionary) -> void:
	...
	if victory and not active_job.is_empty():
		...
		var market_bonus: int = get_room_level("black_market") * 8 if is_room_repaired("black_market") else 0
		...
		jobs_completed += 1
		for crew_id: String in active_crew_ids:
			var rewarded: Dictionary = get_crew_member(crew_id)
			rewarded["xp"] = int(rewarded.get("xp", 0)) + 12 * difficulty
			_apply_crew_level(rewarded)
	...
	active_job.clear()
	active_crew_ids.clear()
	next_job_at = mini(next_job_at, now + 5)
	save_game()
	state_changed.emit()
...
func load_game() -> Dictionary:
	...
	var loaded_active: Variant = data.get("active_job", {})
	active_job = loaded_active as Dictionary if loaded_active is Dictionary else {}
...
func consume_cutscene() -> String:
	var value: String = pending_cutscene
	pending_cutscene = ""
	if value == "prologue":
		intro_seen = true
	save_game()
...
func _story_template(chapter: int) -> Dictionary:
	match chapter:
		1:
			return {"title":"Steal the Ghost Key","sector":"Crater Market Relay","difficulty":1,"target":"Peacekeepers","story":true,"story_id":"ghost_key","chapter":1}
		2:
			return {"title":"Break Blueglass Records","sector":"Blueglass Evidence Vault","difficulty":2,"target":"Peacekeepers","story":true,"story_id":"blueglass","chapter":2}
		3:
			return {"title":"Hijack the Dawn Convoy","sector":"Mare Highway","difficulty":3,"target":"Armored Patrol","story":true,"story_id":"dawn_convoy","chapter":3}
		4:
			return {"title":"Blackout the Precinct","sector":"Authority Grid Seven","difficulty":3,"target":"Peacekeepers","story":true,"story_id":"blackout","chapter":4}
		_:
			return {"title":"Crown the Crater","sector":"Eclipse Signal Tower","difficulty":4,"target":"Peacekeeper Command","story":true,"story_id":"crater_crown","chapter":5}
...
func _add_job(template: Dictionary) -> void:
	...
	var job: Dictionary = {
		"id": job_id,
		"title": String(template.get("title", "Underworld Score")),
		...
		"expires_at": _now() + maxi(32, 62 - heat / 2) + _rng.randi_range(0, 18),
	...
	last_event = "NEW SCORE // %s in %s." % [String(job.get("title", "Score")), String(job.get("sector", "Sector"))]
...
func _complete_story_job(story_id: String, chapter: int) -> void:
	...
	match chapter:
		1:
			pending_cutscene = "ghost_key"
		3:
			pending_cutscene = "war_room"
		5:
			pending_cutscene = "finale"
		_:
			pending_cutscene = ""

func _apply_crew_level(member: Dictionary) -> void:
	var level: int = int(member.get("level", 1))
	var needed: int = level * 80
