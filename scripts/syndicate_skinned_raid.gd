...
## Applies selected threat, crew, victory, defeat, and action skins to tactical raids.
...
func _draw() -> void:
	...
	draw_texture_rect_region(skin_atlas, Rect2(250.0, 220.0, 220.0, 220.0), SyndicateSkins.region(threat_item))
	draw_style_box(_panel(Color(0.0, 0.0, 0.0, 0.0), SyndicateSkins.accent(), 4, 24), Rect2(250.0, 220.0, 220.0, 220.0))
	...
	draw_string(ThemeDB.fallback_font, Vector2(22.0, 112.0), SyndicateSkins.skin_name().to_upper(), HORIZONTAL_ALIGNMENT_RIGHT, 676.0, 9, SyndicateSkins.secondary())
	if battle_over:
...
func _draw_action_button(id_value: String, label: String) -> void:
	...
	draw_style_box(SyndicateSkins.style_box(enabled, 9, 0.96 if enabled else 0.55), rect)
